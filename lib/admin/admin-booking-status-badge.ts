...
export function getAdminBookingStatusBadgeClass(status: string): string {
  ...
  if (s === "final_due") return "bg-blue-100 text-blue-800";
  /** Legacy / migration-only value; may appear on historical booking rows. */
  if (s === "deposit_paid") return "bg-amber-200 text-amber-950";
