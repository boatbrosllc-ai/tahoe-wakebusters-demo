...
import { getDateStrInSlotTimezone, parseSlotIdRelaxed, SLOT_TIMEZONE } from "@/lib/booking/experience-slots";

/** Calendar date (YYYY-MM-DD) for `d` in BUSINESS_TIMEZONE / SLOT_TIMEZONE, not the browser's local zone. */
...
/**
 * Sort key: minutes since business-timezone midnight for this instant.
 */
...
/**
 * Business-local start time in minutes from slotId (hour*60+minute).
 */
...
/** Stable morning→night sort: prefer parsed slot id; fall back to business wall time from ISO. */
