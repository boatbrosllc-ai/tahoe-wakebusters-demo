...
export type AboutSection1Props = {
  ...
  ctaLabel: string;
  ctaHref: string;
  images: [AboutSectionImage, AboutSectionImage, AboutSectionImage, AboutSectionImage];
};

const CLIP_IDS = ["clip-squiggle", "differentone16", "differentone8", "clip-rect"] as const;
const ROTATES = ["rotate-6", "-rotate-6", "-rotate-6", "rotate-6"] as const;
const VARIANT_KEYS = ["up", "down", "down", "down"] as const;
...
export default function AboutSection1({
  ...
  return (
    <section className="relative overflow-hidden bg-brand-bg px-4 pb-20 pt-4 sm:pb-28 sm:pt-6" ref={heroRef}>
      <svg className="absolute -left-[999px] -top-[999px] h-0 w-0">
        <defs>
      ...
      <div className="relative z-10 mx-auto max-w-2xl">
        <div className="mb-6 flex items-center justify-center gap-2 text-sm font-semibold uppercase tracking-[0.18em] text-brand-primary">
          {eyebrow}
        </div>

        <h1 className="mb-6 text-3xl font-semibold text-brand-dark sm:text-4xl md:text-5xl">
          <VerticalCutReveal
            splitBy="words"
            staggerDuration={0.2}
            staggerFrom="last"
            ...
            containerClassName="items-center justify-center text-center font-display font-extrabold leading-[120%] text-brand-dark"
          >
            {title}
          </VerticalCutReveal>
        </h1>
        ...
        <TimelineContent
          ...
          timelineRef={heroRef}
          className="mb-8 text-center text-sm leading-relaxed text-brand-muted sm:text-lg"
        >
          {description}
        </TimelineContent>
        ...
        <TimelineContent
          ...
          customVariants={revealVariants3}
          timelineRef={heroRef}
          className="mx-auto flex w-fit cursor-pointer items-center gap-2 rounded-full border border-brand-secondary bg-brand-secondary px-5 py-3 text-white shadow-lg shadow-brand-secondary/30 transition-all duration-300 ease-in-out hover:gap-4 hover:brightness-110"
        >
          {ctaLabel} <ArrowRight className="h-4 w-4" aria-hidden />
        </TimelineContent>
      </div>

      <div className="mx-auto grid h-[22rem] max-w-6xl grid-cols-2 gap-3 pt-10 sm:h-[16rem] sm:gap-4 sm:pt-14 md:h-[22rem] md:grid-cols-4 lg:h-[26rem]">
        {images.map((image, i) => (
          <TimelineContent
            key={CLIP_IDS[i]}
            as="figure"
            animationNum={2 + i}
            timelineRef={heroRef}
            customVariants={VARIANT_KEYS[i] === "up" ? revealVariants : revealVariants2}
            className="relative h-full w-full overflow-hidden rounded-lg"
            style={{ clipPath: `url(#${CLIP_IDS[i]})` }}
          >
            <Image
              src={image.src}
              alt={image.alt}
              ...
              className={`object-cover object-[center_20%] ${ROTATES[i]}`}
            />
          </TimelineContent>
        ))}
      </div>
